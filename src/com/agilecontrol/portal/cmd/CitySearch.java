...
/**
 * 参照Search, 每个商品数据需要读取翻译表M_PRODUCT_TRANS 进行翻译
 * 
 * > {cmd:"b2b.city.search", 常规搜索条件, c_province_id}
 * 
 * c_province_id* - int 省份id
 * 
 * 对比标准的search方法， { table, exprs, querystr, maxcnt,pagesize, mask, usepref,
 * orderby }
 * 
 * @author stao
 *
...
public class CitySearch extends Search {
	...
	public CmdResult execute(JSONObject jo) throws Exception {
		...
		c_province_id = jo.optInt("c_province_id", -1);
		String cacheKey = jo.optString("cachekey");
		if (Validator.isNotNull(cacheKey)
				&& jedis.exists(cacheKey) /* 需求：若key timeout也要重新查 */) {
			jo.put("table", "city");// cache key wil reload from this table's pk
									// records
		}
	 ...
	 * @param jo
	 *            原始的查询条件，有可能在此地被重构
	 * @return 额外的查询条件，将增加到查询where语句中 key: 是要补充到sql where 部分的clause语句内容，比如
	 *         "emp_id=?" value 是问号对应的实际值，如果value是java.util.List，将允许多值代替？号
	 */
	protected HashMap<String, Object> reviseSearchCondition(JSONObject jo) throws Exception {
		HashMap<String, Object> map = new HashMap<String, Object>();
		StringBuilder sb = new StringBuilder();
		Table table = manager.getTable("city");
