...
> {cmd:"b2b.dim.list", selected, actid, isfav, types, cat, pdtsearch}
...
public class DimList1 extends CmdHandler {
	...
	private JSONArray filterDimValues(String dim,ArrayList<DimValue> dimNodes) throws Exception{
		...
		if(isCart){
			sb.append(" and exists(select 1 from b_cart c where c.b_market_id=m.b_market_id and c.m_product_id=b_mk_pdt.m_product_id and c.user_id=? and c.isactive='Y')");
			params.add(usr.getId());
		}
	...
	public CmdResult execute(JSONObject jo) throws Exception {
		...
		vc.put("1",1);
		vc.put("langid", usrObj.getInt("lang_id"));
	...
	protected void coverVcParams(JSONArray types,VelocityContext vc) throws Exception {
		...
		for (int i = 0; i < types.length(); i++) {
			...
			String paramparam =filter.optString("paramname");
			if(Validator.isNull(paramparam)){
				continue;
			}
			
			//将 客户端传来的值添加到vc中 (防止错误注入值  如 uid=-100, 在 reviseSearchCondition() 中将必要的参数进行覆盖)
			vc.put(paramparam,value);
		}
	}
}
