...
/**
 参照Search, 面向"order", 即: b_bfo

> {cmd:"b2b.order.search", pdtsearch}

返回的结果中，针对每个订单还需要补充订单的第一个商品（就是id最大的那个商品，到sku级别）{id,no,note,mainpic,color,size}

 * @author yfzhu
 *
 */
@Admin(mail="yfzhu@lifecycle.cn")
public class BfoSearch extends Search {
	...
	protected void postAction(JSONObject ret) throws Exception{
		JSONArray ja=ret.getJSONArray("bfo_s");
	...
	protected HashMap<String, Object> reviseSearchCondition(JSONObject jo) throws Exception{
		...
		//作为订单号搜索
		if (Validator.isNotNull(querystr) && querystr.toLowerCase().startsWith("bfo") ) {
			data.put("bfo.docno=?", querystr.toUpperCase());
			return data;
		}
		StringBuilder sb=new StringBuilder();
		...
		if(usr.getLangId()!=LanguageManager.getInstance().getDefaultLangId()){
			sb=new StringBuilder("exists(select 1 from b_bfoitem i, m_product d, M_PRODUCT_TRANS r where i.b_bfo_id=bfo.id and i.m_product_id=d.id and r.m_product_id=d.id and r.b_language_id=?");
			params.add(usr.getLangId());
		}else{
			sb=new StringBuilder("exists(select 1 from b_bfoitem i,m_product d where i.b_bfo_id=bfo.id and i.m_product_id=d.id");
		}
		// search
