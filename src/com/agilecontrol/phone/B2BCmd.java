 ...
 *  首先读取配置文件 /META-INF/conf/cmdmap.properties，格式 webcmd=javaclass
 *  如:
 *  b2b.act.list=com.agilecontrol.b2bweb.cmd.ActList
 *  如果没有定位到，将默认传人的cmd就是class名称直接定位，如果class名称没有"."，将默认在 ad_param#phone.cmd.packages 指定的参数中寻找相应class
 * 
 */
@Admin(mail="yfzhu@lifecycle.cn")
public class B2BCmd extends Command implements PluginLifecycleListener{
	/**
	...
	/**
	 *  /META-INF/conf/cmdmap.properties
	 */
	private Properties cmdMapping;
	
	/**
	 * 根据/META-INF/conf/cmdmap.properties进行cmd映射，如果没有定义映射，返回原值
	 * @param cmdFromWeb cmdmap.properties的key的部分
	 * @return 如果映射未定义，将返回原值
	 */
	private String findCmdByMapping(String cmdFromWeb) throws Exception{
		if(cmdMapping==null){
			String props=PhoneController.getInstance().getResourceAsString("META-INF/conf/cmdmap.properties");
...
//			logger.debug("cmdHandler.allowGuest()="+cmdHandler.allowGuest());
			if(cmdHandler.allowGuest()){
				//change by  cmq 2016-12-20 15:42:06  wine support guest
				...
				try{
					...
					//用户必须属于指定的用户组
					if(Validator.isNotNull( PhoneConfig.B2B_GROUP)){
						int guid=QueryEngine.getInstance().doQueryInt("select gu.id from groupuser gu, groups g where gu.userid=? and gu.groupid=g.id and g.name=?", new Object[]{userId,PhoneConfig.B2B_GROUP}, conn);
						if(guid<0){
							return this.noPermission(event);
						}
					}
...
//			PhoneController.getInstance().shutdown();
			...
			logger.debug("B2BPluginDestroyed");
