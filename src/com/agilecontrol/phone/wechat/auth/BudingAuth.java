...
public class BudingAuth {
	private static final Logger logger = LoggerFactory.getLogger(BudingAuth.class);
	private Connection conn;
	private HttpServletRequest req;
	private HttpServletResponse res;
	private Jedis jedis;//无需关闭
	public BudingAuth(Jedis jedis,Connection conn, HttpServletRequest req, HttpServletResponse res) throws Exception {
		this.jedis=jedis;
		this.conn=conn;
		this.req=req;
		this.res=res;
	...
	public JSONObject accept(JSONObject json) throws Exception {
		...
		if(Validator.isNotNull(nickName)){
			...
			obj.put("openid", json.optString("openid"));
			...
			//现用于注册时的用户昵称展示  wangcun 2016-7-16 13:25:26
		...
		if(one!=null){
			...
			logger.debug("我设置的cookie"+URLEncoder.encode(obj.toString(), "UTF-8"));
			return obj;			
			//create a new user
	...
	private void addRecommender(JSONObject json, JSONObject obj) throws Exception{
		...
		if(Validator.isNull(query) || "undefined".equals(query)){
			return;
		}
